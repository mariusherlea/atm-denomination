...
function denomination(sum, billsValues) {
  ...
  for (let i = 1; i <= billsValues.length; i++) {
    if (
      restAfterDenomination[i - 1] > 0 ||
      restAfterDenomination < billsNumber[i]
    ) {
      billsNumber.push(
        Math.floor(restAfterDenomination[i - 1] / billsValues[i])
      );

      console.log(`bill's number of `, billsValues[i], "=>", billsNumber[i]);

      restAfterDenomination.push(
        restAfterDenomination[i - 1] - billsNumber[i] * billsValues[i]
      );

      console.log(
        "restul dupa impartirea la",
        billsValues[i],
        "=>",
        restAfterDenomination[i]
      );
    } else {
...
$(document).ready(function () {
  $("#btn-add").click(function () {
    ...
    $(
      "table"
    ).append(`<tr><th>${billsValues[0]}</th><th>${billsValues[1]}</th><th>${billsValues[2]}</th><th>${billsValues[3]}</th><th>${billsValues[4]}</th><th>${billsValues[5]}</th></tr>
